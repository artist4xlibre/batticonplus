...
static gboolean get_battery_full_capacity_design (gboolean *use_charge, gdouble *capacity);
...
static gint get_options (int argc, char **argv)
{
	...
	g_option_context_add_main_entries (option_context, option_entries, BATTICONPLUS_STRING);
	...
	if (configuration.display_version == TRUE) {
		g_print (_("batticonplus: a lightweight and fast battery icon that sits in your system tray\n"));
		g_print (_("version %s\n"), BATTICONPLUS_VERSION_STRING);
...
static gboolean changed_power_supplies (void)
{
	...
	gboolean use_charge;
	gdouble full_capacity, full_capacity_design;
	...
	if (directory != NULL) {
		...
		while (file != NULL) {
			...
			if (battery_path != NULL && g_str_has_suffix (battery_path, file) == TRUE && g_str_has_prefix (file, "BAT") == TRUE) {
				num_ps++;
				total_ps++;
			}
	...
	get_battery_full_capacity (&use_charge, &full_capacity);
	get_battery_full_capacity_design (&use_charge, &full_capacity_design);
...
static gboolean get_battery_full_capacity_design (gboolean *use_charge, gdouble *capacity)
{
	...
	sysattr_status = get_sysattr_double (battery_path, "energy_full_design", capacity);
	*use_charge = FALSE;

	if (sysattr_status == FALSE) {
		sysattr_status = get_sysattr_double (battery_path, "charge_full_design", capacity);
		*use_charge = TRUE;
	}
...
static gchar* get_tooltip_string (gchar *battery, gchar *time)
{
	...
	gboolean use_charge;
	gdouble full_capacity, full_capacity_design;
	gdouble remaining_capacity;
	static gchar remaining_capacity_percentage[STR_LTH];
	...
	get_battery_full_capacity (&use_charge, &full_capacity);
	get_battery_full_capacity_design (&use_charge, &full_capacity_design);
	remaining_capacity = (full_capacity * 100) / full_capacity_design;
	g_ascii_dtostr (remaining_capacity_percentage, STR_LTH, remaining_capacity);
	g_sprintf (remaining_capacity_percentage, "%.2f", remaining_capacity);
	...
	g_strlcat (tooltip_string, "\n", STR_LTH);
	g_strlcat (tooltip_string, "[", STR_LTH);
	g_strlcat (tooltip_string, battery_file, STR_LTH);
	g_strlcat (tooltip_string, "] ", STR_LTH);
	g_strlcat (tooltip_string, "Maximum Capacity: ", STR_LTH);
	g_strlcat (tooltip_string, remaining_capacity_percentage, STR_LTH);
	g_strlcat (tooltip_string, "%", STR_LTH);
...
gchar* get_icon_path_from_theme(const gchar *theme_name, const gchar *icon_name)
{
	...
	icon_info = gtk_icon_theme_lookup_icon(theme, icon_name, 48, 0);
...
int main (int argc, char **argv)
{
	...
	setlocale (LC_ALL, "");
	bindtextdomain (BATTICONPLUS_STRING, NLSDIR);
	bind_textdomain_codeset (BATTICONPLUS_STRING, "UTF-8");
	textdomain (BATTICONPLUS_STRING);
	...
	if (configuration.hide_notification == FALSE) {
		if (notify_init (BATTICONPLUS_STRING) == FALSE) {
			return -1;
		}
	}
#endif
